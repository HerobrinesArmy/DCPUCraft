 ...
 * @author Herobrine
 * 
 * The SPC2000 is a deep sleep cell based on the ZEF882 time dilation field generator (available from Polytron Corporation Incorporated). It provides
 * safe and nearly instantaneous time passage, making long journeys in space much easier on the passengers, and allowing cargo to reach its destination
 * with minimal aging occurring. Due to the nature of the ZEF882, it affects the entire vessel (50 meter radius, and will only engage in a near vacuum.
 * Once the SPC2000 is active, the vessel will be almost nowhere to an external observer, and detection of the vessel is beyond unlikely. Because of the
 * strong extra-dimensional acceleration and non-linear temporal distortion that occurs, it's highly recommended that passengers are strapped in and
 * asleep when triggering the SCP2000.
 */
public class VirtualSleepChamber extends DCPUHardware {
   ...
   public VirtualSleepChamber(String id) {
   ...
   public void interrupt() {
      ...
      case ACTION_SET_SKIP_UNIT:
         ...
         numberOfUnitsToSkip = ByteBuffer.allocate(8).order(ByteOrder.BIG_ENDIAN)
               .putLong((dcpu.ram[b] << 48) | (dcpu.ram[b + 1] << 32) | (dcpu.ram[b + 2] << 16) | dcpu.ram[b + 3]).order(ByteOrder.LITTLE_ENDIAN).getLong(0);
         break;
