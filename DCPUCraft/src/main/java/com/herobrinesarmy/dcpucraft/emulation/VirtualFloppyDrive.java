...
/**
 * Experimental, partial implementation of the MF35D Floppy Drive
 * 
 * @author Herobrine
 * 
 * The Mackapar 3.5" Floppy Drive is compatible with all standard 3.5" 1440 KB floppy disks. The floppies need to be formatted in 16 bit mode, for a
 * total of 737,280 words of storage. Data is saved on 80 tracks with 18 sectors per track, for a total of 1440 sectors containing 512 words each. The
 * M35FD works is asynchronous, and has a raw read/write speed of 30.7kw/s. Track seeking time is about 2.4 ms per track.
 * 
   ...
   public void interrupt() {
      ...
      if (a == 0) {
         ...
         if (sector <= MAX_SECTOR && (state == STATE_READY || state == STATE_READY_WP)) {
            operation = new FloppyOperation(FloppyOperation.READ, sector, dcpu.registers[4], dcpu.cycles + READ_CYCLES_PER_SECTOR + SEEK_CYCLES_PER_TRACK
         ...
         if (sector <= MAX_SECTOR && state == STATE_READY) {
            operation = new FloppyOperation(FloppyOperation.WRITE, sector, dcpu.registers[4], dcpu.cycles + WRITE_CYCLES_PER_SECTOR + SEEK_CYCLES_PER_TRACK
                  * Math.abs(track - (sector / SECTORS_PER_TRACK)));
            dcpu.registers[1] = 1;
            setState(STATE_BUSY);
         } else {
